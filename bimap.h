...
public:
    ...
    struct iterator
    {
        ...
        iterator(avl_node<typename node_traits<left_t, right_t, half>::value_type>* avl_node_ptr)
            : node_ptr { static_cast<node<left_t, right_t>*>(!avl_node_ptr ? nullptr : avl_node_ptr->origin) }
        { }

        iterator(node<left_t, right_t>* node_ptr)
            : node_ptr { node_ptr }
        { }
        ...
        {
            iterator<left_t, right_t, half> tmp { node_ptr };
            operator++();
            return tmp;
        }
        ...
        iterator<left_t, right_t, half>& operator--()
        {
            auto ptr { node_traits<left_t, right_t, half>::get_avl_node(node_ptr).prev() };
            node_ptr = static_cast<node<left_t, right_t>*>(!ptr ? nullptr : ptr->origin);
            return *this;
        }
        ...
        {
            return iterator<left_t, right_t, !half>(node_ptr);
        }
    ...
    {
        return left_tree.find(v);
    }

    right_iterator find_right(const right_t& v) const
    {
        return right_tree.find(v);
    }

    left_iterator begin_left() const
    {
        return left_iterator { left_tree.find_min() };
    }
    ...
    {
        return right_iterator { right_tree.find_min() };
    }
